...
void MultiStepRefiner::Refine()
{
	...
	// step1: outlier detection（利用左右一致性检查，筛选遮挡点和误匹配点）
	if (do_lr_check_) {
		OutlierDetection();
	}
	// step2: iterative region voting（局部迭代投票，投票统计支撑区域的最优，用来替代无效值）
	if (do_region_voting_) {
		IterativeRegionVoting();
	}
	// step3: proper interpolation（视差填充，16个方向搜索，对于遮挡像素，用视差最小的候选点来填充，对于误匹配点，用颜色最近的候选点来填充）
...
void MultiStepRefiner::OutlierDetection()
{
	...
	// 遮挡区像素和误匹配区像素
	auto& occlusions = occlusions_;     // 由于前景遮挡，在左视图中可见而在右视图中不可见的点
	...
	for (sint32 y = 0; y < height; y++) {
		for (sint32 x = 0; x < width; x++) {
			...
			if (col_right >= 0 && col_right < width) { // 判断右视图对应点坐标的有效性
				// 右影像上同名像素的视差值
				const auto& disp_r = disp_right_[y * width + col_right];    // 获取右视图的视差值
				// 判断两个视差值是否一致（差值在阈值内）
				if (abs(disp - disp_r) > threshold) {
					...
					// else 
					//		pixel in mismatches
					const sint32 col_rl = lround(col_right + disp_r);   // 通过右视图坐标获取左视图对应坐标
					if (col_rl > 0 && col_rl < width) {
						const auto& disp_l = disp_left_[y * width + col_rl];    // 获取右到左该点的视差值
						if (disp_l > disp) {    // p因为遮挡而在右影像上不可见，所以它会匹配到右影像上的前景像素，而前景像素的视差值必定比背景像素大。
...
void MultiStepRefiner::IterativeRegionVoting()
{
	...
	for (sint32 it = 0; it < num_iters; it++) {
		for (sint32 k = 0; k < 2; k++) {
			...
			for (auto& pix : trg_pixels) {
                ...
                if(it%2==0){
                    ...
                    for (sint32 t = -arm.left; t <= arm.right; t++) {
                        const sint32& xt = x + t;
                        auto& arm2 = arms[y * width_ + xt];
                        for (sint32 s = -arm2.top; s <= arm2.bottom; s++) {
                            const auto& d = disp_left_[(y+s) * width + xt];
                            if (d != Invalid_Float) {
                                const auto di = lround(d);
                                histogram[di - min_disparity_]++;
                            }
			...
			// 删除已填充像素
			for (auto iter = trg_pixels.begin(); iter != trg_pixels.end();) {
				const sint32 x = iter->first;
				const sint32 y = iter->second;
				if(disp_left_[y * width + x]!=Invalid_Float) {
					iter = trg_pixels.erase(iter);
				}
				else { ++iter; }
			}
		}
			...
			if (e_label == 1) {
				...
				if (d != Invalid_Float) {
					...
					for (int k = 0; k<2; k++) {
                        ...
                        // 如果左右点的视差值是有效值
						if (d2 != Invalid_Float) {
                            // 如果该像素点前后点的代价比该像素点的代价小，就用代价更低的点的视差来填充边缘点的视差
                            // cost_ptr[-disp_range] 代表前一个点的视差最小时的代价，加上偏移量d2i表示前一个点视差为d2i时的匹配代价值
