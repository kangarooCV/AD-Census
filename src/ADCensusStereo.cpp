...
void ADCensusStereo::ComputeDisparity()
{
	...
	// 为了加快读取效率，把单个像素的所有d的代价值存储到局部数组里
	...
	for (sint32 i = 0; i < height; i++) {
		for (sint32 j = 0; j < width; j++) {
			...
			// ---遍历视差范围内的所有代价值，输出当前像素最小代价值及对应的视差值
			for (sint32 d = min_disparity; d < max_disparity; d++) {
				const sint32 d_idx = d - min_disparity;     // 视差值减去最小视差才是视差维度的索引号
				const auto& cost = cost_local[d_idx] = cost_ptr[i * width * disp_range + j * disp_range + d_idx];   // 在这一步顺便把该像素所有视差的代价值取出来，后面要读取最小视差前后视差的代价值
				if (min_cost > cost) {
					min_cost = cost;
					best_disparity = d;
				}
			...
			// ---子像素拟合
			if (best_disparity == min_disparity || best_disparity == max_disparity - 1) {   // 如果最佳视差在边界，那就直接返回，不需要亚像素拟合
...
void ADCensusStereo::ComputeDisparityRight()
{
    ...
    // 注：假设d的定义就是 d= 左-右
    // 右cost(3,2,1)就是 右图的(3, 2)和左图的(3+1, 2)求匹配代价
    // 左cost(4,2,1)就是 左图的(4, 2) 和右图的 (4-1, 2)求匹配代价
    // 因此有 右cos(xr, yr, d) 就是 右图的(xr yr) 和左图的 (xr+d, yr)求匹配代价
    // 也有   左cost(xr+d, yr) 就是 左图的(xr+d, yr) 和右图的 (xr, yr)求匹配代价，都是相同的两个点求代价
    // 因此有 右cost(xr,yr,d) = 左cost(xr+d,yl,d)
	for (sint32 i = 0; i < height; i++) {
		for (sint32 j = 0; j < width; j++) {
			float32 min_cost = Large_Float;
			sint32 best_disparity = 0;
